...
This update captures the last-3-days changes (2026-04-11 to 2026-04-13), including hosting prep and follow-up game/options/profile adjustments.

### Git Window (Last 3 Days)
- `16e74b6` (2026-04-11): `trying to host`
- `68e5a7d` (2026-04-11): `firebase stff`

### Hosting / Deployment Notes
- Vercel deployment troubleshooting identified a critical env naming rule:
  - Firebase web config values must be stored as:
  ...
  - Using raw keys like `apiKey` / `projectId` does not work with current app code.
- Firebase config hardening in `src/app/firebase/config.ts`:
  - trims common pasted quote/comma formatting
  - throws explicit errors for missing vars
  - validates API key format (`AIza...`) early.

### Debug UI Gating (Prod vs Dev)
- Game debug UI gating standardized with `NEXT_PUBLIC_DEBUG_UI` and `NODE_ENV` behavior in play pages.
- `src/app/options/page.tsx` behavior now split as:
  - `Reset Lessons` + `Reset Game Scores`: available in production and dev
  - second panel (`Complete Beginner/Inter/Advanced`): debug-only.

### Profile Navigation Fix
- Back buttons on profile pages were changed to true history-back behavior with fallback:
  - `router.back()` when history exists
  - fallback to `/Home` when opened directly.
- Files:
  - `src/app/profile/page.tsx`
  - `src/app/profile/[uid]/page.tsx`

### Code Fixer Data/Prompt Adjustments
- `src/data/code-fixer-levels.json` updates:
  - Q4: `a = 8` (keeps expected output `2` with `%`)
  - Q22: `a = 8`, expected output changed to `5` (with `-`)
  - Q23/Q24 prompt changed to relational wording:
    - `Add the missing relational operator (greater,lesser):`
  - Q25/Q26 prompt changed to logical wording:
    - `Add the missing logical operator (or, and, etc):`
- `src/app/play/code_fixer/page.tsx` prompt highlighter extended:
  - `logical` now renders red
  - `relational` now renders red.

### Current Production Checklist
1. Keep Vercel Firebase env vars with exact `NEXT_PUBLIC_...` names.
2. Add production domain(s) in Firebase Auth authorized domains.
3. Redeploy after every env change.
4. Open deployment explicitly marked `Production` when validating fixes.
